          ...
          const container = qs('#siteFileTree'); if(container){
            ...
            function renderNode(node, parentEl){
              ...
              if(node.type === 'dir'){
                ...
                label.appendChild(caret); label.appendChild(icon); label.appendChild(text);
                ...
                nodeEl.appendChild(label); nodeEl.appendChild(childrenWrap);
                (node.children||[]).forEach(ch=> renderNode(ch, childrenWrap));
              } else {
                const fileRow = document.createElement('div'); fileRow.style.display='flex'; fileRow.style.alignItems='center'; fileRow.style.gap='8px';
                ...
                fileRow.appendChild(icon); fileRow.appendChild(nameSpan); nodeEl.appendChild(fileRow);

                nodeEl.addEventListener('click', async (ev)=>{ ev.stopPropagation(); const sel = qs('#pageSelect'); if(sel) sel.value = node.path; setSelectedFile(nodeEl, node); await loadPageIntoEditor(node.path, selectedSite.name); });
              }
            ...
            try {
              const current = qs('#pageEditor')?.getAttribute('data-current-page');
              if(current){
                const nodes = container.querySelectorAll('.sf-node');
                for(const nEl of nodes){ if(nEl && nEl.title === current){ nEl.classList.add('selected'); window.__currentSelectedFileEl = nEl; const info = qs('#siteActions'); if(info) info.textContent = `Selected: ${current} — File`; break; } }
              }
            } catch(e){}
          }
